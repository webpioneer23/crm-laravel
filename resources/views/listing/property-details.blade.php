<form method="post" action="{{route('listing.update', $listing->id)}}">
    ...
    <div class="row g-3 mb-3">
        <div class="col-sm-6">
            ...
            <select name="bedrooms" class="select2 form-select" id="bedrooms">
                <option value="-1" {{$listing->bedrooms == -1 ? 'selected' : ''}}>studio</option>
        ...
        <div class="col-sm-6">
            ...
            <select name="bathrooms" class="select2 form-select" id="bathrooms">
                @for ($i = 0; $i < 51; $i++) <option value="{{$i}}" {{$listing->bathrooms == $i ? 'selected' : ''}}>{{$i}}</option>
                    @endfor
            </select>
        </div>
